...
constexpr int kLabeledFieldGap = 6;
constexpr int kMinimumLabeledFieldWidth = 24;

// MeasureLeadingLabelReserve returns the horizontal space that K input-like
// widgets need to keep their FLTK left-side label inside the caller-supplied
// rectangle. FLTK draws labels outside the effective control box, so the K
// wrappers reserve that space before constructing the native field.
int MeasureLeadingLabelReserve(const char* label, int total_width) {
    if (!label || label[0] == '\0' || total_width <= kMinimumLabeledFieldWidth) {
        return 0;
    }

    fl_font(FL_HELVETICA, kWidgetFontSize);
    const int measured = static_cast<int>(std::ceil(fl_width(label))) + kLabeledFieldGap;
    return std::max(0, std::min(measured, total_width - kMinimumLabeledFieldWidth));
}

int LabeledFieldX(int x, int w, const char* label) {
    return x + MeasureLeadingLabelReserve(label, w);
}

int LabeledFieldW(int w, const char* label) {
    return std::max(1, w - MeasureLeadingLabelReserve(label, w));
}
...
KInput::KInput(int x, int y, int w, int h, const char* label)
    : Fl_Input(LabeledFieldX(x, w, label), y, LabeledFieldW(w, label), h, label) {
...
KTextBox::KTextBox(int x, int y, int w, int h, const char* label)
    : Fl_Multiline_Input(LabeledFieldX(x, w, label), y, LabeledFieldW(w, label), h, label) {
    ApplyInputStyle(this);
}
...
void KTabs::draw_tab(int x1, int x2, int W, int H, Fl_Widget* child, int flags, int selected) {
    ...
    const int tab_y = y();
    // FLTK's selected argument is not a reliable boolean for every tab in all
    // overflow/layout modes. The active child returned by value() is the stable
    // single source of truth, so only that page receives selected colors.
    const bool is_selected = child == value();
...
KChoice::KChoice(int x, int y, int w, int h, const char* label)
    : Fl_Choice(LabeledFieldX(x, w, label), y, LabeledFieldW(w, label), h, label) {
    box(KThemeInputBoxtype());
    textfont(FL_HELVETICA);
    textsize(kWidgetFontSize);
    ApplyWidgetTextStyle(this);
...
KOutput::KOutput(int x, int y, int w, int h, const char* label)
    : Fl_Output(LabeledFieldX(x, w, label), y, LabeledFieldW(w, label), h, label) {
    ApplyInputStyle(this);
}
