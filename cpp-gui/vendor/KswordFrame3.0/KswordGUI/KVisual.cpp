...
void KSwitch::draw() {
    const KTheme& t = KThemeManager::instance().theme();
    const Fl_Color accent = ResolveAccent(accent_color_);
    const int track_w = std::min(w(), std::max(42, h() * 2));
    const int track_h = std::max(1, h() - 6);
    const int text_x = x() + track_w + 8;
    const int text_w = std::max(0, x() + w() - text_x);

    fl_push_clip(x(), y(), w(), h());

    // Redraw the whole widget rectangle before drawing the new state. Without
    // this clear, switching between longer and shorter labels leaves old glyphs
    // behind, which looks like "Enabled" and "Disabled" overlap.
    Fl_Color clear_color = parent() ? parent()->color() : t.panelBg;
    fl_color(clear_color);
    fl_rectf(x(), y(), w(), h());

    fl_color(checked_ ? accent : (hover_ ? t.hover : t.controlAltBg));
    fl_rectf(x(), y() + 3, track_w, track_h);
    DrawBorder(x(), y() + 3, track_w, track_h, checked_ ? accent : t.border);

    const int knob = std::max(10, h() - 12);
    const int knob_x = checked_ ? x() + track_w - knob - 5 : x() + 5;
    const int knob_y = y() + (h() - knob) / 2;
    fl_color(t.panelBg);
    fl_rectf(knob_x, knob_y, knob, knob);
    DrawBorder(knob_x, knob_y, knob, knob, t.border);

    DrawText(checked_ ? on_text_ : off_text_, text_x, y(), text_w, h(), checked_ ? accent : t.mutedText, FL_ALIGN_LEFT);
    fl_pop_clip();
}
