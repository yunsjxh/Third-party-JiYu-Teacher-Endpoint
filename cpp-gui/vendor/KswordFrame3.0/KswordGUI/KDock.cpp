...
class KDockFloatingWindow : public Fl_Double_Window {
public:
    // Creates a borderless top-level host and stores the dock that owns it. The
    // host is intentionally frameless: the KDockWidget draws its own title bar
    // (with close/float/pin buttons) at client (0,0), and the drag math positions
    // the window at root-cursor minus the grab offset. A native caption would push
    // the client area down by the caption height, so the grabbed title would trail
    // the cursor by that amount ("not following the mouse"). Frameless keeps
    // client == window, so the dock stays exactly under the cursor while dragging.
...
// KDockGuideRect is a tiny rectangle helper used by the docking guide hit tests.
// Inputs and outputs stay in the caller's coordinate system, so the same helper
// works for screen-space hit testing and FLTK draw-space painting.
struct KDockGuideRect {
    int x;
    int y;
    int w;
    int h;
...
// ContainsGuidePoint checks whether a point lies inside one guide button. Inputs
// must use the same coordinate space; the output is true for an inclusive-left,
// exclusive-right rectangle hit.
bool ContainsGuidePoint(const KDockGuideRect& rect, int px, int py) {
    return px >= rect.x && px < rect.x + rect.w && py >= rect.y && py < rect.y + rect.h;
}

// RootGuideRect returns one of the four main-window edge guide buttons. Inputs
// describe the manager box; output is the button rectangle for hit/draw logic.
KDockGuideRect RootGuideRect(int box_x, int box_y, int box_w, int box_h, KDockDropSide side) {
    const int safe_w = std::max(kDropGuideSize, box_w);
    const int safe_h = std::max(kDropGuideSize, box_h);
    const int center_x = box_x + safe_w / 2 - kDropGuideSize / 2;
    const int center_y = box_y + safe_h / 2 - kDropGuideSize / 2;
    switch (side) {
    case KDockDropSide::Left:
        return { box_x + kDropGuideInset, center_y, kDropGuideSize, kDropGuideSize };
    case KDockDropSide::Right:
        return { box_x + safe_w - kDropGuideInset - kDropGuideSize, center_y, kDropGuideSize, kDropGuideSize };
    case KDockDropSide::Top:
        return { center_x, box_y + kDropGuideInset, kDropGuideSize, kDropGuideSize };
    case KDockDropSide::Bottom:
        return { center_x, box_y + safe_h - kDropGuideInset - kDropGuideSize, kDropGuideSize, kDropGuideSize };
    default:
        return { 0, 0, 0, 0 };
    }
}

// PaneGuideRect returns the clustered guide buttons for a single leaf pane. The
// center button means tabify; side buttons split that pane in the chosen direction.
KDockGuideRect PaneGuideRect(int box_x, int box_y, int box_w, int box_h, KDockDropSide side) {
    const int safe_w = std::max(kDropGuideSize, box_w);
    const int safe_h = std::max(kDropGuideSize, box_h);
    const int gap = 6;
    const int center_x = box_x + safe_w / 2 - kDropGuideSize / 2;
    ...
    switch (side) {
    case KDockDropSide::Center:
        return { center_x, center_y, kDropGuideSize, kDropGuideSize };
    case KDockDropSide::Left:
        return { center_x - kDropGuideSize - gap, center_y, kDropGuideSize, kDropGuideSize };
    case KDockDropSide::Right:
        return { center_x + kDropGuideSize + gap, center_y, kDropGuideSize, kDropGuideSize };
    case KDockDropSide::Top:
        return { center_x, center_y - kDropGuideSize - gap, kDropGuideSize, kDropGuideSize };
    case KDockDropSide::Bottom:
        return { center_x, center_y + kDropGuideSize + gap, kDropGuideSize, kDropGuideSize };
    default:
        return { 0, 0, 0, 0 };
...
// GuideSideAtRootSet returns the explicit root guide under a point, or None.
// Unlike the old nearest-edge heuristic, this only docks when the user is over
// a visible guide button.
KDockDropSide GuideSideAtRootSet(int px, int py, int box_x, int box_y, int box_w, int box_h) {
    const KDockDropSide sides[] = {
        KDockDropSide::Left,
        KDockDropSide::Right,
        KDockDropSide::Top,
        KDockDropSide::Bottom
    };
    for (KDockDropSide side : sides) {
        if (ContainsGuidePoint(RootGuideRect(box_x, box_y, box_w, box_h, side), px, py)) {
            return side;
        }
    }
    return KDockDropSide::None;
}

// GuideSideAtPaneSet returns the explicit pane guide under a point, or None.
// The center guide is checked first so a tabify drop cannot be stolen by a
// neighboring split guide if a very small pane makes the cluster tight.
KDockDropSide GuideSideAtPaneSet(int px, int py, int box_x, int box_y, int box_w, int box_h) {
    const KDockDropSide sides[] = {
        KDockDropSide::Center,
        KDockDropSide::Left,
        KDockDropSide::Right,
        KDockDropSide::Top,
        KDockDropSide::Bottom
    ...
    for (KDockDropSide side : sides) {
        if (ContainsGuidePoint(PaneGuideRect(box_x, box_y, box_w, box_h, side), px, py)) {
            return side;
        }
    }
    return KDockDropSide::None;
...
// DrawRootGuideSet paints only the four root-edge docking buttons. Inputs are a
// manager rectangle and an active side; there is no return value.
void DrawRootGuideSet(int box_x, int box_y, int box_w, int box_h, KDockDropSide active_side) {
    KDockGuideRect left = RootGuideRect(box_x, box_y, box_w, box_h, KDockDropSide::Left);
    KDockGuideRect right = RootGuideRect(box_x, box_y, box_w, box_h, KDockDropSide::Right);
    KDockGuideRect top = RootGuideRect(box_x, box_y, box_w, box_h, KDockDropSide::Top);
    KDockGuideRect bottom = RootGuideRect(box_x, box_y, box_w, box_h, KDockDropSide::Bottom);
    DrawDropGuide(left.x, left.y, "L", active_side == KDockDropSide::Left);
    DrawDropGuide(right.x, right.y, "R", active_side == KDockDropSide::Right);
    DrawDropGuide(top.x, top.y, "T", active_side == KDockDropSide::Top);
    DrawDropGuide(bottom.x, bottom.y, "B", active_side == KDockDropSide::Bottom);
}

// DrawPaneGuideSet paints the five clustered pane buttons. Inputs are a pane
// rectangle and active side; the function returns no value.
void DrawPaneGuideSet(int box_x, int box_y, int box_w, int box_h, KDockDropSide active_side) {
    KDockGuideRect center = PaneGuideRect(box_x, box_y, box_w, box_h, KDockDropSide::Center);
    KDockGuideRect left = PaneGuideRect(box_x, box_y, box_w, box_h, KDockDropSide::Left);
    KDockGuideRect right = PaneGuideRect(box_x, box_y, box_w, box_h, KDockDropSide::Right);
    KDockGuideRect top = PaneGuideRect(box_x, box_y, box_w, box_h, KDockDropSide::Top);
    KDockGuideRect bottom = PaneGuideRect(box_x, box_y, box_w, box_h, KDockDropSide::Bottom);
    DrawDropGuide(center.x, center.y, "C", active_side == KDockDropSide::Center);
    DrawDropGuide(left.x, left.y, "L", active_side == KDockDropSide::Left);
    DrawDropGuide(right.x, right.y, "R", active_side == KDockDropSide::Right);
    DrawDropGuide(top.x, top.y, "T", active_side == KDockDropSide::Top);
    DrawDropGuide(bottom.x, bottom.y, "B", active_side == KDockDropSide::Bottom);
...
void KDockWidget::draw() {
    ...
    const int title_h = isFloating() ? kTitleHeight : 0;

    // Draw the panel body first so hidden content never leaks through stale
    // chrome. Docked widgets rely on KDockArea's tab strip for their title, so
    // only floating widgets draw this internal title bar.
    ...
    if (title_h > 0) {
        // Floating title bar carries drag semantics and close/float/pin buttons.
        bool active = true;
        Fl_Color title_fill = fl_color_average(kColorPrimary(), kColorWindow(), 0.22f);
        fl_color(title_fill);
        fl_rectf(x() + 1, y() + 1, std::max(0, w() - 2), title_h - 1);
        fl_color(kColorPrimary());
        fl_rectf(x() + 1, y() + 1, 3, title_h - 1);
        fl_color(kColorBorder());
        fl_line(x(), y() + title_h, x() + w() - 1, y() + title_h);
        ...
        fl_color(kColorText());
        fl_font(FL_HELVETICA_BOLD, 12);
        fl_draw(title_.c_str(), title_left, y(), text_w, title_h, FL_ALIGN_LEFT | FL_ALIGN_CENTER | FL_ALIGN_INSIDE);
...
int KDockWidget::handle(int event) {
    ...
    const bool title_bar_enabled = isFloating();
    if (title_bar_enabled && event == FL_PUSH && Fl::event_button() == FL_LEFT_MOUSE) {
...
void KDockWidget::closeDock() {
    ...
    if (floating_window_) {
        ...
        // closeDock() may run from a title-button FL_PUSH handler inside the
        // floating host. Defer host destruction so FLTK never resumes an event
        // dispatch on a window that was deleted by its child handler.
...
void KDockWidget::layoutContent() {
    ...
    // Docked widgets have no internal title bar because KDockArea already draws
    // the tab strip. Floating widgets keep a title bar for drag/close controls.
    const int title_h = isFloating() ? kTitleHeight : 0;
    int client_x = x() + kMargin;
    int client_y = y() + title_h + kMargin;
    int client_w = std::max(kMinClientSize, w() - kMargin * 2);
    int client_h = std::max(kMinClientSize, h() - title_h - kMargin * 2);
...
void KDockWidget::createFloatingWindowAt(int root_x, int root_y, int width, int height) {
    ...
    floating_window_->show();
    floating_window_->take_focus();
    // Do not delete empty dynamic areas here. This method is often called from
    // KDockArea::handle(FL_DRAG); deleting that area while its event handler is
    // still on the stack is a plausible cause of intermittent access violations.
    // The manager performs safe cleanup on drop/close after the drag leaves the
    // source area's event stack.
    if (active_manager) {
        active_manager->layoutAreas();
        active_manager->redraw();
    }
...
void KDockManager::dockWidgetFromDrag(KDockWidget* widget, int root_x, int root_y) {
    ...
    // Structural docking can delete empty dynamic source areas. Clear transient
    // drag pointers before that cleanup so no overlay state keeps stale area or
    // dock pointers alive after the tree is rewritten.
...
void KDockManager::cleanupEmptyAreas() {
    // Cleanup can be requested by programmatic moves, close operations, or the
    // end of a drag. The rewrite deliberately collapses only split-tree nodes
    // here and never destroys a KDockArea widget while FLTK may still be
    // unwinding an input/draw stack for that area. Dynamic area widgets stay in
    // all_areas_ as hidden reusable shells until restore/destruction.
    if (dragging_widget_) {
        return;
    }

    auto subtree_empty = [](KDockLayoutNode* node) -> bool {
        std::function<bool(KDockLayoutNode*)> visit = [&](KDockLayoutNode* current) -> bool {
            if (!current) {
                return true;
            }
            if (current->is_leaf) {
                return !current->area || current->area->dockCount() <= 0;
            }
            return visit(current->first) && visit(current->second);
        };
        return visit(node);
    };

    std::function<bool(KDockLayoutNode*&, KDockLayoutNode*)> collapse =
        [&](KDockLayoutNode*& node, KDockLayoutNode* parent) -> bool {
        if (!node) {
            return false;
        }
        node->parent = parent;
        ...
        bool changed = collapse(node->first, node) || collapse(node->second, node);
        const bool first_empty = subtree_empty(node->first);
        const bool second_empty = subtree_empty(node->second);
        if (!first_empty && !second_empty) {
            return changed;
        }

        KDockLayoutNode* old_node = node;
        KDockLayoutNode* keep = nullptr;
        KDockLayoutNode* discard = nullptr;
        if (first_empty && !second_empty) {
            keep = old_node->second;
            discard = old_node->first;
        }
        else if (!first_empty && second_empty) {
            keep = old_node->first;
            discard = old_node->second;
        }
        else {
            discard = old_node;
        }
        ...
        old_node->first = nullptr;
        old_node->second = nullptr;
        node = keep;
        deleteLayoutTree(discard);
        delete old_node;
        return true;
    };

    collapse(root_node_, nullptr);
    if (!root_node_ || subtree_empty(root_node_)) {
        deleteLayoutTree(root_node_);
        root_node_ = new KDockLayoutNode(center_);
    }
    root_node_->parent = nullptr;
    ...
    for (KDockArea* dock_area : all_areas_) {
        ...
        if (!areaIsInLayout(dock_area) || dock_area->dockCount() <= 0) {
            dock_area->hide();
            dock_area->resize(0, 0, 1, 1);
        }
    }
...
KDockDropTarget KDockManager::dropTargetForRootPoint(KDockWidget* widget, int root_x, int root_y) const {
    ...
    const KDockDropSide root_side = GuideSideAtRootSet(root_x, root_y, manager_root_x, manager_root_y, manager_w, manager_h);
    if (root_side != KDockDropSide::None) {
        target.valid = true;
        target.scope = KDockDropScope::MainWindow;
        target.side = root_side;
        target.position = SideToPosition(root_side);
        target.area = area(target.position);
        return target;
    }

    // Find the leaf area under the pointer. The area is retained on the target
    // even when the pointer is not over a guide button so drawDropOverlay() can
    // still paint the pane-local guide cluster without implying a valid drop.
    for (KDockArea* dock_area : all_areas_) {
        if (!dock_area || !dock_area->visible() || dock_area->dockCount() <= 0) {
            continue;
        }
        KDockWidget* dock = dock_area->activeDockWidget();
        if (!dock || dock == widget || dock->isFloating()) {
            continue;
        }
        int area_root_x = 0;
        int area_root_y = 0;
        int area_w = 0;
        int area_h = 0;
        WidgetRootBox(dock_area, area_root_x, area_root_y, area_w, area_h);
        if (root_x < area_root_x || root_x >= area_root_x + area_w ||
            root_y < area_root_y || root_y >= area_root_y + area_h) {
            continue;
        }
        ...
        target.side = GuideSideAtPaneSet(root_x, root_y, area_root_x, area_root_y, area_w, area_h);
        target.valid = target.side != KDockDropSide::None;
        return target;
    }
    return target;
}
KDockDropTarget KDockManager::dropTargetForFloatingWidget(KDockWidget* widget, int root_x, int root_y) const {
    ...
    if (target.valid || target.scope != KDockDropScope::None || target.area || !widget || !widget->isFloating()) {
        return target;
    }
    int center_x = 0;
    int center_y = 0;
...
void KDockManager::drawDropOverlay() {
    if (!dragging_widget_ && !active_drop_target_.valid) {
        return;
    }
    // Main-window guides remain visible for context. Pane-local guides are drawn
    // only over the hovered leaf so the user explicitly chooses tabify/split
    // instead of triggering a nearest-edge heuristic.
    const KDockDropSide main_side = active_drop_target_.scope == KDockDropScope::MainWindow
        ? active_drop_target_.side
        : KDockDropSide::None;
    DrawRootGuideSet(x(), y(), w(), h(), main_side);
    ...
    if (active_drop_target_.scope != KDockDropScope::DockWidget || !active_drop_target_.area) {
        return;
    }
    KDockArea* target_area = active_drop_target_.area;
    if (!target_area->visible()) {
        return;
    }
    const KDockDropSide pane_side = active_drop_target_.valid ? active_drop_target_.side : KDockDropSide::None;
    if (active_drop_target_.valid) {
        DrawDropPreview(target_area->x(), target_area->y(), target_area->w(), target_area->h(), active_drop_target_.side);
    }
    DrawPaneGuideSet(target_area->x(), target_area->y(), target_area->w(), target_area->h(), pane_side);
}
void KDockManager::layoutNode(KDockLayoutNode* node, int node_x, int node_y, int node_w, int node_h) {
    ...
    if (node->is_leaf) {
        ...
        // Only the final empty root leaf is visible as a neutral placeholder.
        // Empty leaves inside a split are collapsed by cleanupEmptyAreas(); while
        // a drag is still active they stay hidden instead of painting blank panes.
        bool should_show = (node == root_node_) || dock_area->dockCount() > 0;
