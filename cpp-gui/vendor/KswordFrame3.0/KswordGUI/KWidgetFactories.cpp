...
// PopupMenuItemBinding keeps the clicked item's stable index separate from
// Fl_Widget::user_data(). FLTK stores callback data in the same user_data slot,
// so using user_data for both the index and callback state overwrites the index.
struct PopupMenuItemBinding {
    PopupMenuState* state = nullptr;
    int index = -1;
};

// PopupMenuWindow closes when the user clicks outside its bounds.
class PopupMenuWindow : public Fl_Double_Window {
public:
...
void PopupMenuItemCallback(Fl_Widget* widget, void* data) {
    (void)widget;
    PopupMenuItemBinding* binding = static_cast<PopupMenuItemBinding*>(data);
    PopupMenuState* state = binding ? binding->state : nullptr;
    if (!binding || !state || !state->window) {
        return;
    }
    state->selected = binding->index;
    state->window->hide();
}

// IsUsableMessageOwner returns true for a visible top-level window that can be
// used as the owner rectangle for transient dialogs. Iconified windows are
// shown but not visible in FLTK, so they are intentionally ignored.
bool IsUsableMessageOwner(Fl_Window* window) {
    return window && window->shown() && window->visible();
}

// TopWindowFromWidget resolves an arbitrary focused/hovered widget to its
// visible top-level window, which is the best owner for parentless message APIs.
Fl_Window* TopWindowFromWidget(Fl_Widget* widget) {
    if (!widget) {
        return nullptr;
    }

    Fl_Window* top = widget->top_window();
    return top ? top : widget->window();
}

// Message dialogs are parentless for API compatibility, so infer the best owner
// from current FLTK interaction state and then fall back to the first visible
// application window. This keeps KShowMessage() centered over the window that
// triggered it instead of appearing at the process/default top-left position.
Fl_Window* BestMessageOwnerWindow() {
    Fl_Window* owner = TopWindowFromWidget(Fl::focus());
    if (IsUsableMessageOwner(owner)) {
        return owner;
    }

    owner = TopWindowFromWidget(Fl::belowmouse());
    if (IsUsableMessageOwner(owner)) {
        return owner;
    }
    ...
    for (Fl_Window* window = Fl::first_window(); window; window = Fl::next_window(window)) {
        if (IsUsableMessageOwner(window)) {
            return window;
        }
    }
    return nullptr;
...
void ClampToScreen(int& x, int& y, int w, int h, int probe_x, int probe_y) {
    int screen_x = 0;
    int screen_y = 0;
    int screen_w = 0;
    int screen_h = 0;
    Fl::screen_xywh(screen_x, screen_y, screen_w, screen_h, probe_x, probe_y);
    if (screen_w <= 0 || screen_h <= 0) {
        return;
    }
    x = std::max(screen_x, std::min(x, screen_x + std::max(0, screen_w - w)));
    y = std::max(screen_y, std::min(y, screen_y + std::max(0, screen_h - h)));
}

void CenterDialogForKShowMessage(Fl_Window& dialog) {
    if (Fl_Window* owner = BestMessageOwnerWindow()) {
        const int center_x = owner->x_root() + owner->w() / 2;
        const int center_y = owner->y_root() + owner->h() / 2;
        int target_x = center_x - dialog.w() / 2;
        int target_y = center_y - dialog.h() / 2;
        ClampToScreen(target_x, target_y, dialog.w(), dialog.h(), center_x, center_y);
        dialog.position(target_x, target_y);
        return;
    }

    int probe_x = 0;
    int probe_y = 0;
    Fl::get_mouse(probe_x, probe_y);
    int screen_x = 0;
    int screen_y = 0;
    int screen_w = 0;
    int screen_h = 0;
    Fl::screen_xywh(screen_x, screen_y, screen_w, screen_h, probe_x, probe_y);
    int target_x = screen_x + (screen_w - dialog.w()) / 2;
    int target_y = screen_y + (screen_h - dialog.h()) / 2;
    ClampToScreen(target_x, target_y, dialog.w(), dialog.h(), probe_x, probe_y);
    dialog.position(target_x, target_y);
}
}
...
void KShowMessage(const char* title, const char* message) {
    Fl_Double_Window dialog(380, 170, title ? title : "Message");
    dialog.set_modal();
    // A transient stack window must not be registered for global theme refresh:
    // once it is destroyed the manager would keep a dangling pointer. Apply the
    // current theme once instead of routing through SetWindowStyle().
    KThemeManager::instance().ApplyTo(&dialog);
    const KTheme& theme = KThemeManager::instance().theme();
    dialog.color(theme.panelBg);
    dialog.labelcolor(theme.text);
    dialog.redraw();
    dialog.begin();
    KText msg(20, 20, 340, 75, message ? message : "");
    msg.box(FL_FLAT_BOX);
    msg.color(theme.panelBg);
    msg.labelcolor(theme.text);
    ...
    CenterDialogForKShowMessage(dialog);
    dialog.show();
...
int KShowPopupMenu(int x, int y, const std::vector<std::string>& items) {
    ...
    // Transient popup: style it once without registering it for global refresh,
    // so its stack pointer can never dangle inside KThemeManager::RefreshAll().
    KThemeManager::instance().ApplyTo(&menu_window);
    menu_window.redraw();
    menu_window.set_modal();
    PopupMenuState state;
    state.window = &menu_window;
    std::vector<PopupMenuItemBinding> bindings(items.size());
    ...
    for (std::size_t i = 0; i < items.size(); ++i) {
        ...
        bindings[i].index = static_cast<int>(i);
        button->callback(PopupMenuItemCallback, &bindings[i]);
    }
    menu_window.end();
