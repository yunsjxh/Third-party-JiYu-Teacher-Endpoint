...
void ApplyValuatorPalette(Fl_Valuator* valuator, const KTheme& theme) {
    ...
    valuator->box(KThemeInputBoxtype());
...
void ApplyTextDisplayPalette(Fl_Text_Display* text_display, const KTheme& theme) {
    ...
    text_display->box(KThemeInputBoxtype());
...
void KThemeManager::RefreshAll() {
    // Only persistent top-level windows are ever registered here: transient
    // dialogs and popup menus style themselves directly through ApplyTo() and
    // never call RegisterWindow(), so windows_ cannot hold a dangling pointer
    // (that was the theme-toggle access violation). Drop only defensive nulls,
    // then restyle and repaint each registered window so a theme mode or accent
    // change applies globally instead of only to the control that was clicked.
    //
    // Note: this must NOT filter by Fl::first_window(). RefreshAll() runs once
    // during startup before the main window is show()n, when FLTK reports no live
    // windows; erasing "not yet shown" entries there would permanently drop the
    // main window and leave later toggles with nothing to refresh.
...
void KThemeManager::RebuildTheme() {
    ...
    if (mode_ == KThemeMode::Light) {
        theme_.windowBg = MakeColor(255, 255, 255);
        theme_.panelBg = MakeColor(255, 255, 255);
        theme_.controlBg = MakeColor(255, 255, 255);
        theme_.controlAltBg = MakeColor(255, 255, 255);
        ...
        theme_.windowBg = MakeColor(0, 0, 0);
        theme_.panelBg = MakeColor(0, 0, 0);
        theme_.controlBg = MakeColor(0, 0, 0);
        theme_.controlAltBg = MakeColor(0, 0, 0);
...
namespace {
// DrawFlatBorderBox fills the interior with the widget color and strokes a single
// pixel of the active theme border. It reads the border color live so a light/dark
// or accent change repaints correctly, and keeps controls visible even when their
// fill matches the panel behind them (white input on a white card).
void DrawFlatBorderBox(int x, int y, int w, int h, Fl_Color c) {
    fl_color(c);
    fl_rectf(x, y, w, h);
    fl_color(KThemeManager::instance().theme().border);
    fl_rect(x, y, w, h);
}
}

Fl_Boxtype KThemeInputBoxtype() {
...
void InitFlatThemeGlobal() {
    ...
    // Register the shared flat-border box (1px inset) before any themed control is
    // constructed or drawn, so inputs/sliders/choices are visible from the first paint.
    Fl::set_boxtype(KThemeInputBoxtype(), DrawFlatBorderBox, 1, 1, 2, 2);
