...
KCommandPalette::KCommandPalette(int x, int y, int w, int h, const char* label)
    : Fl_Double_Window(x, y, w, h, label ? label : "Command Palette"), commands_(), query_(), placeholder_("Type a command"), active_index_(0), selected_command_(-1) {
...
void KInspectorPanel::draw() {
    ...
    for (int i = 0; i < static_cast<int>(sections_.size()) && cy < y() + h(); ++i) { const KInspectorSection& section = sections_[i]; fl_color(t.controlBg); fl_rectf(x() + 1, cy, w() - 2, header_height_); DrawText(section.expanded ? "-" : "+", x() + 8, cy, 16, header_height_, accent, FL_ALIGN_CENTER); DrawText(section.title, x() + 30, cy, w() - 40, header_height_, t.text, FL_ALIGN_LEFT, kTitleFont); cy += header_height_;
