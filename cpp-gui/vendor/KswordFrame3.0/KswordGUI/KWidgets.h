...
public:
    ...
    // Enables or disables the built-in context menu with "copy cell" and
    // "copy row" actions. Input is the desired state; no value is returned.
    void set_builtin_copy_context_menu_enabled(bool enabled);

    // Returns whether the built-in copy menu is active for cells without a
    // caller-provided context callback.
    bool builtin_copy_context_menu_enabled() const;

    // Copies one cell's text to the system clipboard. Inputs are logical row
    // and column indexes; return value is true only when a valid cell existed.
    bool copy_cell_to_clipboard(int row, int col) const;

    // Copies one full row as tab-separated text to the system clipboard. Input
    // is the logical row index; return value is true only when the row existed.
    bool copy_row_to_clipboard(int row) const;
...
private:
    ...
    // Shows the built-in copy menu and returns true when it handled the event.
    bool show_builtin_copy_context_menu(int row, int col);
...
private:
    ...
    int context_menu_x_;
    int context_menu_y_;
    bool builtin_copy_context_menu_enabled_;
};
